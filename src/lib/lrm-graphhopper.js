(function () {
  ...
  L.Routing.GraphHopper = L.Evented.extend({
    ...
    route: function (waypoints, callback, context, options) {
      ...
      options = options || {};
      url = this.buildRouteUrl(waypoints, options); // Keep the base URL the same

      // Create a copy of the waypoints to avoid async modifications
          ...
          if (!timedOut) {
            ...
            if (xhr.status === 200) {
              try {
                ...
                data.paths = [data.paths[0]]; // Retain only the first path
      ...
      var requestBody = {
        points: [
          [-73.82165634707668, 40.8476371],
          [-73.8216873768893, 40.8443659],
        ],
        profile: "foot",
        elevation: true,
        instructions: true,
